...
import android.content.Context;
	...
	public class Toast extends JDialog{
		...
		private final int CHARACTER_LENGTH_MULTIPLIER = 12;
	    ...
	    private void createGUI(){
	        ...
	        add(label);
	        int width= mText.length() * CHARACTER_LENGTH_MULTIPLIER;
	        int height = 25;
	        setBounds((mOwner.getWidth()-width)/2, mOwner.getHeight()/2, width, height);
	    }
		...
		public static Toast makeText(Context owner, String text){
			return makeText(owner, text, LENGTH_SHORT);
		}
		
		public static Toast makeText(Context owner, String text, Style style){
			return makeText(owner, text, LENGTH_SHORT, style);
		}
	    
	    public static Toast makeText(Context owner, String text, int duration){
	    	return makeText(owner, text, duration, Style.NORMAL);
	    }
	    
	    public static Toast makeText(Context owner, String text, int duration, Style style){
	    	Toast toast = new Toast(owner.getJFrame());
	    	toast.mText = text;
